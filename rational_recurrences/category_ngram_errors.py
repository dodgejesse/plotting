...
def main():
    get_category_data = False
    data, categories, worst, best, learned_structures = load_errors.get_data(get_category_data = get_category_data)
    print(data)
    ...
    for d_out in [24]:#[["24", "6,6,6,6"],["256","64,64,64,64"]]:
        one_plot(worst, best, data, categories, train_nums, d_out=d_out, colors=colors, get_category_data = get_category_data)
...
def one_plot(worst, best, data, categories, train_nums, d_out, colors, get_category_data):
    plt.figure()
    ...
    for pattern in data[d_out]:
        for sparsity in data[d_out][pattern]:
            ...
            if x_categorical:
                ...
                adjustment = (pattern_counter / 5.0 - .5)/2.0
    ...
    save_plot(d_out, get_category_data)

def save_plot(d_out, get_category_data):
    if get_category_data:
        save_loc = "plot_drafts/category_errors/error_by_category_dout={}_hparamopt.pdf".format(d_out)
    else:
        save_loc = "plot_drafts/category_errors/error_dout={}_hparamopt.pdf".format(d_out)
